...
namespace AddParserSrv.Classes
{
    public static class Parser
    {
        ...
        public const string AptReg = "(( iş[ ]han.*[ ])|( is[ ]han.*[ ])|( iş[ ]m.*[ ])|( is[ ]m.*[ ])|( bina.*[ ])|" +
            ...
            ")";
        public const string CadReg = "(( yol.*[ ])|( c[ ])|( c[. ])|( cd[ ])|( cds[ ])|( cd[. ])|( cad[ ])|( cad[. ])|( cadde.*[ ]))";
        public const string SiteReg = "(( st[ ])|( st[. ])|( site.*[ ])|( lojm.*[ ]))";
        public const string BlokReg = "(( blok[ ])|( blok[-]))";
        ...
        public const string DaireReg = "(( d[.])|( d[.:])|( d[:])|( da[.])|( da[.:])|( da[:])|( daire[:])|( daire[ ])|( d[-])|( d[ ]))";
        ...
        public static AddressDT ParseAddress(string addressStr)
        {
            ...
            try
            {
                ...
                var dict = GetSearchDict(tmpAddr);
                ...
                {
                    var word = FindAddressPart(tmpAddr, rulledMatches, tmpMatch, item.Value);
                    switch (item.Value)
                    {
                        case SearchType.Mah:
                            addr.Mahalle = word[0];
        ...
        {
            if (address.Contains("e. mudur.")) address.Replace("e. mudur.", "emniyet mudurlugu ");
        ...
        private static string ChangeTurkishToEnglish(string turkish)
        {
            ...
            return addressCorrection(turkish.Replace("ş", "s")
           .Replace("ı", "i")
           .Replace("ö", "o")
           .Replace("ü", "u")
           .Replace("ğ", "g")
        ...
        private static SuggestionDT SpellCheck(string word, string cities, string districts)
        {
            ...
            return word == "" ? new SuggestionDT() : retVal;
        }
        ...
        private static string[] FindAddressPart(string addressStr, string rulledMatches, string tmpMatch, SearchType sType)
        {
            ...
            if (addressStr.Length > repWord.Length)
            {
                ...
                repWord = "";
            if (matched.Count > 0)//cagatay edit
            {
